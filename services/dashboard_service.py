...
class DashboardService:
    ...
    def get_executive_kpis(_self) -> Dict[str, Any]:
        """Calculates core campus KPIs from the engineered dataset."""
        df = _self.data_service.load_dataset()
    ...
    @st.cache_data(ttl=300)
    def get_building_rankings(_self) -> List[Dict[str, Any]]:
    ...
    @st.cache_data(ttl=300)
    def calculate_campus_health_score(_self) -> Dict[str, Any]:
        """
        ...
        """
        df = _self.data_service.load_dataset()
        settings = _self.data_service.settings_manager.load_settings()
        ...
        # 2. HVAC Utilization Score
        temp_threshold = settings.get("hvac_temp_threshold")
        hvac_waste_records = df[
            (df["Running ACs"] > 2)
            & ((df["Temperature"] < temp_threshold) | (df["Occupancy"] < 5))
        ]
        waste_ratio = len(hvac_waste_records) / len(df)
        hvac_score = max(0.0, 100.0 - (waste_ratio * 150))

        # 3. Occupancy Waste Score (Idle Load)
        idle_threshold = settings.get("idle_load_threshold_kw")
        idle_waste_records = df[
            (df["Occupancy"] == 0) & (df["Energy Consumption"] > idle_threshold)
        ]
    ...
    def generate_executive_brief(self) -> str:
        ...
        if health["power_factor_score"] < 85:
            warnings_list.append("low average power factor compliance")
        if health["hvac_efficiency_score"] < 80:
            warnings_list.append("inefficient HVAC scheduling during off-peak temperatures")
        if health["occupancy_efficiency_score"] < 80:
            warnings_list.append("energy leaks in unoccupied rooms (idle load waste)")

        if warnings_list:
            brief += (
                "**Critical Management Action Needed:** Immediate maintenance attention is recommended regarding "
                + ", ".join(warnings_list)
                + "."
            )
        else:
